...
# make_sidebar()
# from pages import funcao_Rota_Vcasada
import streamlit as st
# Proteção de acesso
...
# Função para processar e criar Grafico de barras
def gerar_mapa_calor(df, titulo, dias_validos, data_inicio, data_fim):
    ...
    # Agrupa diretamente para calcular totais entrega por hora
    df_agrupado = (
        df.groupby(['HORA', 'DIA_DA_SEMANA'], observed=False)['ROTA_HORARIO_REALIZADO']
        .count().reset_index(name='TOTAL_ENTREGAS')  # Contar diretamente o número de entregas
        .sort_values(by=['HORA', 'DIA_DA_SEMANA'])
    )

    # Agrupa diretamente e criar grafico de barras para calcular por hora a quantidade de entregas <= 40 minutos
    df['ENTREGAS_40'] = df['TEMPO_ENTREGA'].apply(lambda x: 1 if x <= timedelta(minutes=40) else 0)
    df_agrupado_40 = (
        df.groupby(['HORA', 'DIA_DA_SEMANA'], observed=False)['ENTREGAS_40']
        .sum()  # Somar diretamente o número de entregas <= 40 minutos
    ...
    tabela_entregas_40 = df_agrupado_40.pivot(index='HORA', columns='DIA_DA_SEMANA', values='ENTREGAS_40')

    for dia in dias_semana:
        if dia not in tabela_entregas_40.columns:
            tabela_entregas_40[dia] = 0

    tabela_entregas_40 = tabela_entregas_40[dias_validos]

    df_agrupado = df_agrupado.pivot(index='HORA', columns='DIA_DA_SEMANA', values='TOTAL_ENTREGAS')

    for dia in dias_semana:
        if dia not in df_agrupado.columns:
            df_agrupado[dia] = 0

    df_agrupado = df_agrupado[dias_validos]

    # Gráfico de barras para totais de entrega por hora
    fig, ax = plt.subplots(figsize=(10, 6))
    df_agrupado.sum(axis=1).plot(kind='bar', ax=ax)
    ax.set_title('Totais de entrega por hora')
    ax.set_xlabel('Hora')
    ax.set_ylabel('Total de Entregas')
    st.pyplot(fig)

    # Gráfico de barras para totais de entrega <= 40 minutos por hora
    fig, ax = plt.subplots(figsize=(10, 6))
    df_agrupado_40.groupby('HORA')['ENTREGAS_40'].sum().plot(kind='bar', ax=ax)
    ax.set_title('Totais de entrega menor ou igual 40 minutos ')
    ax.set_xlabel('Hora')
    ax.set_ylabel('Total de Entregas <= 40 minutos')
    st.pyplot(fig)
...
def main():
    ...
    # Seleção de loja
    loja_selecionada = st.selectbox("Selecione a loja", options=loja_dict.keys(), format_func=lambda x: loja_dict[x], key="Qnav1_loja")

    # Navegação por barra
    navegacao = st.radio("Filtrar periodo", options=["Ano", "Mês", "Semana"], key="Qnav1_navegacao")

    if navegacao == "Ano":
        anos = obter_ultimos_anos()
        ano_selecionado = st.selectbox("Selecione o ano", options=anos, key="Qnav1_ano")

        data_inicio_ano = datetime(ano_selecionado, 1, 1)
        data_fim_ano = datetime(ano_selecionado, 12, 31)

        st.write(f"Grafico de barra para o ano {ano_selecionado}")

        query = gerar_query_dados(data_inicio_ano, data_fim_ano, loja_selecionada)
        # query = gerar_query_dados_vr(data_inicio_ano, data_fim_ano, loja_selecionada)

        df = executar_query(engine, query)
        # dfvr = executar_query_vr(engine, query)
        if not df.empty:
            gerar_mapa_calor(df, f"Grafico de barra para - Ano {ano_selecionado}", dias_semana, data_inicio_ano, data_fim_ano)
        else:
            st.warning("Nenhum dado encontrado para o ano selecionado.")

    elif navegacao == "Mês":
        anos = obter_ultimos_anos()
        ano_selecionado = st.selectbox("Selecione o ano", options=anos, key="Qnav2_ano")
        meses = obter_meses()
        mes_selecionado = st.selectbox("Selecione o mês", options=meses, key= "Qnav2_mes")
        ...
        st.write(f"Grafico de barra para {mes_selecionado}/{ano_selecionado}")

        query = gerar_query_dados(data_inicio_mes, data_fim_mes, loja_selecionada)
        df = executar_query(engine, query)
        if not df.empty:
            gerar_mapa_calor(df, f"Grafico de barra - {mes_selecionado}/{ano_selecionado}", dias_validos, data_inicio_mes, data_fim_mes)
        else:
            st.warning("Nenhum dado encontrado para o mês selecionado.")

    elif navegacao == "Semana":
        anos = obter_ultimos_anos()
        ano_selecionado = st.selectbox("Selecione o ano", options=anos, key="Qnav3_ano")
        meses = obter_meses()
        mes_selecionado = st.selectbox("Selecione o mês", options=meses, key="Qnav3_mes")
        ...
        st.write(f"Grafico de barra para a semana {semana_selecionada} de {mes_selecionado}/{ano_selecionado}")

        query = gerar_query_dados(data_inicio_semana, data_fim_semana, loja_selecionada)
        df = executar_query(engine, query)
        if not df.empty:
            gerar_mapa_calor(df, f"Grafico de barra - Semana {semana_selecionada} - {mes_selecionado}/{ano_selecionado}", dias_validos, data_inicio_semana, data_fim_semana)
        else:
